...
- **[PRIORIDADE — parcial]** controles de volume:
  - (b) ✅ FEITO — slider de **volume de saída/peers** > 1.0: per-peer `GainNode` (Web Audio) substituindo `audioElement.volume` (que trava em 1). `audioPrefs` persiste; `SpatialAudio.setPeerGain` ajusta ao vivo; slider no painel 🎧.
  - (a) ⏳ PENDENTE — ganho do **microfone** > 1.0: exige rebuildar o pipeline de publish (Web Audio antes do publishTrack), e conflita com o toggle de mic (`setMicrophoneEnabled`) e o switch de device (`switchActiveDevice`) — risco alto na fragilidade do áudio. Decidir abordagem (TrackProcessor do LiveKit vs. dono do pipeline) antes de implementar.
