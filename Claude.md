...
- ✅ FEITO (2026-05-17) — **porta da Segurança nunca abre + aviso**: `tickDoors` trata `door-security_room` à parte — força `open=false` sempre (não abre por proximidade como as outras) e, se um player fica "na frente" (insiste), manda `room:error` ("🔒 Área restrita — só a Segurança tem acesso") com throttle por player (`lastBlockedNotify`, 2s) pra não floodar. Server-only. NPC guarda não usa porta/colisão, não afetado.
