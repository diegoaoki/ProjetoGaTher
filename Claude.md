...
- **[pedido 2026-05-17] não mostrar "você tem uma mesa reservada" ao entrar**: remover o toast/aviso de mesa reservada no join (poluição).
- **[pedido 2026-05-17] bolha sem convite**: remover o fluxo de convite de bolha (🫧 invite/aceitar — `bubble:invite`/`bubble:respond`); clicar pra abrir bolha cria a bolha direto com a pessoa.
- **[pedido 2026-05-17] sentar na cadeira ao chegar**: verificar viabilidade — quando o avatar chega na cadeira, tocar anim/pose de sentado (os spritesheets `${id}_sit` do LimeZu já são carregados em `AssetLoader.preloadLimezuAssets`/`createCharacterAnimations`; falta detectar "chegou na cadeira", orientar e trocar pra anim `sit`, e voltar a `idle/walk` ao mover).
...
- 🐞 **[BUG diagnosticado 2026-05-17, fix não aplicado] recusar pedido de acesso a sala trancada não expulsa o convidado**: `handleAccessRespond` (accepted=false) chama `ejectFromRoom()` que seta `player.x/y` no server, mas o jogo é *authoritative-light* (cliente é dono da posição e sobrescreve no próximo `move`) → a pessoa continua dentro. O cliente recebe `access:response{roomId,accepted:false}` mas só mostra toast (App.tsx ~753), não reposiciona. **Fix**: incluir `x,y` do eject no `access:response` (server `handleAccessRespond`/`ejectFromRoom`) e no cliente, quando `accepted===false`, chamar `sceneRef.current?.forceTeleport(x,y)` — mesma solução do bug do visitante (`661ce0d`). ~2 arquivos, sem mudança de schema. O fluxo de convite 👋 (`invite`/`invite:respond`) está correto — só o do cadeado tem o problema.
...
- ✅ FEITO (2026-05-17) — **workstations por departamento (desk + PC)**: fallback escolhido pelo user (sprites ricos do print não localizados no pack). 4 PNGs `client/public/assets/interiors/desks/desk_{dev,dados,infra,fin}.png` (64×64) = tampo de madeira (`coffeeTable.png` #20) + 2 monitores (tile `13,3`) com a tela tintada na cor do setor (Dev azul / Dados verde / Infra laranja / Fin roxo). `AssetLoader.DESK_SPRITES`. **`FurnitureItem.tex`** (override só de textura — `type` continua `"desk"` pra reserva/overlay/spawn/`allDesks` não quebrarem); `drawFurniture` e o editor usam `item.tex || item.type`. `addWorkstation` define `tex` por `tileY` (Dev y<11, Dados<21, Infra<31, Fin) e **removeu o `monitor` separado** (vem embutido). office_1/office_2 (diretorias) mantêm desk+monitor antigos.
...
- **[pedido 2026-05-17] área verde ao redor do mapa (sem acesso)**: moldura externa decorativa (gramado) em volta do prédio, opcional com ruas e árvores. Só estética, área inalcançável pelo avatar (borda do mundo já existe).
