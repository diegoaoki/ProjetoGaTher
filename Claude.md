...
- editor de mapas
- **[pedido 2026-05-18] cor nas paredes editável no map editor**: `Wall` ganha `color?` (default = `0x3d4a5e` atual); `drawWalls`/`renderEditWalls` aplicam; no editor, seletor de cor por parede selecionada (e/ou cor do "pincel parede"). Persiste no `map_layout` (override já guarda `walls`).
- **[pedido 2026-05-18] categorias/busca na paleta do editor**: a paleta de móveis cresceu muito (cozinha/segurança/desks/etc) — agrupar por categoria + campo de busca pra achar o item.
...
- **[pedido 2026-05-18] sprint com Shift**: andando (WASD) + segurar Shift dobra a velocidade. Atenção ao `MAX_DELTA` (anti-cheat server, 600px) e ao boost por tempo já existente (`speedMul` cresce 0.5x a cada 3s, cap 3x) — combinar/limitar pra não estourar o delta.
