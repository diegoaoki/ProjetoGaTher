...
- 🟢 [FEITO] **porta da Segurança não abre + aviso ao insistir** — `tickDoors` força `door-security_room` sempre fechada (não abre por proximidade) e manda `room:error` ("Área restrita") com throttle de 2s/player se a pessoa fica na frente. Server-only.
