...
- 🟢 [FEITO] **workstations por departamento (desk+PC)** — fallback (sprites do print não achados): 4 PNGs `desks/desk_{dev,dados,infra,fin}.png` (tampo madeira + 2 monitores tintados por setor); `DESK_SPRITES`; `FurnitureItem.tex` (type continua "desk" → reserva intacta); `addWorkstation` define tex por tileY e removeu o monitor separado.
