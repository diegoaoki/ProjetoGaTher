...
- 🐞 [BACKLOG 2026-05-19, pedido do user] **chat não é responsivo no mobile** — o `ChatPanel` no celular está ruim/quebrado. Revisar: largura/altura (usa `width:"100vw"` quando `mobile`, mas `panelStyle` é `position:absolute top/right/bottom` 340px fixo), abas (Geral/Aqui/DMs) apertadas, input/teclado virtual cobrindo a thread, área de mensagens sem `100dvh`/safe-area, lista de DMs e reações tocáveis. Provável: fazer o painel ocupar a tela toda no mobile (fullscreen como a sidebar já faz), `100dvh` + `env(safe-area-inset-*)`, input fixo no rodapé acima do teclado, fontes/touch targets maiores. Conferir `ChatPanel.tsx` `panelStyle`/`mobile` e o gate de abertura no `App.tsx`.
